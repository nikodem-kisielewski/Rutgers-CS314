...
class data_analysis:
    ...
    def normalize_series(self,data_series):
        ...
        # Use reduce function to find the sum of all the elements of data_series
        total = ft.reduce(lambda x, y: x+y, data_series)

        # Normalize the values of data_series and return it as a new list
        normalized_series = list(map(lambda num: num/total, data_series))
    ...
    def compute_average_across_series(self,list_of_series):
        ...
        sum_list = [sum(num) for num in zip(*list_of_series)]
        
        div = len(list_of_series)
        average_list = list(map(lambda num: num/div, sum_list))
    ...
    def compute_correlations(self,data_series_1,data_series_2):
        ...
        #### FILL IN CODE HERE ####
        # Find average for both lists
        avg1 = ft.reduce(lambda x, y: x+y, data_series_1)/len(data_series_1)
        avg2 = ft.reduce(lambda x, y: x+y, data_series_2)/len(data_series_2)

        # Subtract averages from each element of the 2 lists
        sub1 = list(map(lambda n: n-avg1, data_series_1))
        sub2 = list(map(lambda n: n-avg2, data_series_2))

        # Multiply corresponding elements of the 2 lists
    ...
    def compute_net_correlation(self,data_series_1,data_series_2):
        ...
        net_correlation = ft.reduce(lambda x, y: x+y, correlation_list)
    ...
    def compute_average_of_series(self,data_series):
        ...
        #### FILL IN CODE HERE ####
        series_average = ft.reduce(lambda x, y: x+y, data_series)/len(data_series)
    ...
    def compute_standard_deviation_of_series(self,data_series,average_of_series):
        ...
        '''
        #### FILL IN CODE HERE ####
        sub = list(map(lambda n: (n-average_of_series) ** 2, data_series))

        series_standard_deviation = (ft.reduce(lambda x, y: x+y, sub)/len(data_series)) ** 0.5
    ...
    def question2(self):
        ...
        #### FILL IN CODE #### START
        for i in range(0, len(countries_raw)):
            if gdppc_raw[i] > gdp_per_capita_threshold:
                high_net_birth_rate_countries.append(countries_raw[i])
            else:
                low_net_birth_rate_countries.append(countries_raw[i])
        #### FILL IN CODE #### END
        ...
        lowGDP = []
        for i in range(0, len(countries_raw)):
            if countries_raw[i] in high_net_birth_rate_countries:
                highGDP.append(gdppc_raw[i])
            else:
                lowGDP.append(gdppc_raw[i])
        high_net_birthrate_gdppc_average = self.compute_average_of_series(highGDP)
        high_net_birthrate_gdppc_std = self.compute_standard_deviation_of_series(highGDP, high_net_birthrate_gdppc_average)
        low_net_birthrate_gdppc_average = self.compute_average_of_series(lowGDP)
        low_net_birthrate_gdppc_std = self.compute_standard_deviation_of_series(lowGDP, low_net_birthrate_gdppc_average)
        #### FILL IN CODE #### END
    ...
    def question3(self,energy_consumption_average_IN,ratio_kids_survive_over_kids_fatality_IN):
        ...
        #### FILL IN CODE #### START
        inc = 0.11/9
        t1 = [0.01 + inc*x for x in range(10)]
        #### FILL IN CODE #### END
